...
"""
class UGVSpamBot(IRCBot):
    def __init__(self):
        # Call the parent constructor to connect to the IRC server and join the channel
        IRCBot.__init__(self, "UTA_SpamBot", "irc.libera.chat", "#RTXDrone")

    # Send fire message to channel
    def send_fire_message(self, team_name, location):
        # Format the message with the given information
        # TODO: Verify that this message format is correct
        while True:
            id_fake = random.randint(120, 300)
            message = "RTXDC_2023 {}_UGV_Hit_{}_{}".format(
                team_name, id_fake, location)

            spam_delay = random.randint(1, 3)
...
while True:
    ...
    uav_bot.send_fire_message(team_name, location)
    ...
    if len(corners) > 0:
        ...
        for (markerCorner, markerID) in zip(corners, ids):
            ...
            if markerID not in markerID_list:
                ...
                # Send fire message to server
                uav_bot.send_fire_message(team_name, current_time, location)
